...
- The Vercel deployment overrides the defaults to **Gemini**: `LLM_BASE_URL=https://generativelanguage.googleapis.com/v1beta/openai`, `LLM_MODEL=gemini-3.6-flash`, key in `GLM_API_KEY`. (OpenRouter free models rate-limit hard; Groq blocks browser CORS.) Gemini retires flash model ids fast — a 404's response body (now surfaced in the thrown error) names the current one. `HTTP-Referer`/`X-Title` go out only when `LLM_BASE_URL` is OpenRouter — other providers reject unknown headers at CORS preflight.
